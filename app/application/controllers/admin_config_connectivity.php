...
class admin_config_connectivity extends Home
{
    ...
    public function edit_config()
    {
        ...
        if ($_POST) 
        {
            ...
            $this->form_validation->set_rules('recaptcha_site_key',       '<b>'.$this->lang->line("recaptcha site key").'</b>',     'trim|xss_clean|required');
            $this->form_validation->set_rules('recaptcha_secret_key',     '<b>'.$this->lang->line("recaptcha secret key").'</b>',   'trim|xss_clean|required');
            ...
            else 
            {
                ...
                $recaptcha_site_key=addslashes(strip_tags($this->input->post('recaptcha_site_key', true)));
                $recaptcha_secret_key=addslashes(strip_tags($this->input->post('recaptcha_secret_key', true)));

                $update_data=array("google_api_key"=>$google_api_key,"recaptcha_secret_key"=>$recaptcha_secret_key,"recaptcha_site_key"=>$recaptcha_site_key);
                $insert_data=array("google_api_key"=>$google_api_key,"recaptcha_secret_key"=>$recaptcha_secret_key,"recaptcha_site_key"=>$recaptcha_site_key);
